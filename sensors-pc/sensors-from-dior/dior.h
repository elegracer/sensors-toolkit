...
struct AttitudeCsv {
    ...
    void load(const std::string &filename) {
        ...
        if (FILE *csv = fopen(filename.c_str(), "r")) {
            ...
            fscanf(csv, "%2047[^\r\n]%*[\r\n]", header_line);
            AttitudeData item;
            while (!feof(csv) && fscanf(csv, "%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf[^\r\n]%*[\r\n]", &item.t, &item.g.x, &item.g.y, &item.g.z, &item.atti.x, &item.atti.y, &item.atti.z, &item.atti.w) == 8) {
                items.emplace_back(std::move(item));
            }
            fclose(csv);
        }
    ...
    void save(const std::string &filename) const {
        if (FILE *csv = fopen(filename.c_str(), "w")) {
            fputs("#t[s:double],g.x[m/s^2:double],g.y[m/s^2:double],g.z[m/s^2:double],atti.x[double],atti.y[double],atti.z[double],atti.w[double]\n", csv);
            for (auto item : items) {
                fprintf(csv, "%.14e,%.9e,%.9e,%.9e,%.9e,%.9e,%.9e,%.9e\n", item.t, item.g.x, item.g.y, item.g.z, item.atti.x, item.atti.y, item.atti.z, item.atti.w);
            }
            fclose(csv);
        }
    }
